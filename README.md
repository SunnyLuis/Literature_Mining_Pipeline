...
这是一个用于自动化检索、下载并解析生物医学文献的管道工具。该管道通过对接 NCBI PubMed 数据库和 PubTator 平台，实现基于关键字的大规模文献智能分析与实体提取（包括基因、物种、化学物质、疾病等）。
...
### 2. 生物实体标注数据下载 (`02.get_article_jason_from_pmid.py`)
* **功能**：读取 `pmid_list.txt`，将 PMID 分批次（默认每批 10 个）向 PubTator API 发送请求，下载包含丰富生物实体标注的 biocJSON 原始数据。
* **特点**：
  * 控制请求延迟（`REQUEST_DELAY = 0.5s`）避免被 API 屏蔽。
  * 保存为原始响应 JSON 文件 `raw_api_responses.json`。

### 3. 数据解析与结构化导出 (`03.extract_article_details_from_jason.py`)
* **功能**：解析 `raw_api_responses.json`，提取文献基本元数据和生物实体数据，生成结构化的 TSV 文件。
* **提取字段**：
  * **基本元数据**：PMID、Journal、Year、DOI、PMCID、Authors、Title、Abstract。
  * **标注实体**：Species（物种及ID）、Gene（基因名及ID）、Chemical（化学物质及ID）、Disease（疾病及ID）。
  * **关系标注**：Relations_Display（实体间相互关系展示）。
